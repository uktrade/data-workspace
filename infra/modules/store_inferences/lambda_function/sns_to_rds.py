...
def process_message(record: dict[str, Any]) -> None:
    try:
        ...
        input_payload_file.split("s3://")[1].split("/")[0]
        "/".join(input_payload_file.split("s3://")[1].split("/")[1:])

        logger.info("Now retrieving password from secrets manager")
        datasets_db_password = sm.get_secret_value(
            SecretId=DATASETS_DB_SECRET_ARN,
        )["SecretString"]
